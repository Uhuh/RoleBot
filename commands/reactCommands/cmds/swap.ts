  ...
  run: (message: Message, args: string[], client: RoleBot) => {
    ...
    if (roleNames.length > 2) {
      return message.channel.send(
        'Too many role names given. Only give two when running the command. Example `rb reaction swap Xbox | PS4`'
      );
    } else if (roleNames.length !== 2) {
      return message.channel.send(
        'Not enough roles given. Try again, example `rb reaction swap Xbox | PS4`'
      );
    }
    ...
    if (roles.size !== 2) {
      ...
      if (!role)
        return message.channel.send('No roles found. Check for typos.');

      return message.channel.send(
        `Could only find \`${role.name}\`. Check for typos for the other.`
      );
    }
    ...
    if (reactionRoles[0].folder_id && reactionRoles[1].folder_id) {
      const folder = client.folderContents.get(
        Number(reactionRoles[0].folder_id)
      );
      const secFolder = client.folderContents.get(
        Number(reactionRoles[1].folder_id)
      );
      if (!folder || !secFolder) {
...
const roleFolderSwap = (
  ...
  if (!removeRole) {
    return console.error(
      `${role.role_name} : Couldn't find role in ${currFolder.label}.`
    );
  }
