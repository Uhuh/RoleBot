...
export default {
  ...
  run: (message: Message, args: string[], client: RoleBot) => {
    ...
    const [roleName, folderId] = args.join(' ').split('|').map(l => l.trim().toLowerCase());
    ...
    if(!role) {
      return message.channel.send(`Could not find \`${roleName}\`. Check for typos!`);
    }

    const reactionRole = reactionById(role.id);

    if(reactionRole.folder_id === folderId) {
      return message.channel.send(`Didn't move role as you gave it the the same spot to move to.`)
    } else {
      ...
      if (folderId && (Number.isNaN(ARRAY_ID) || ARRAY_ID < 0 || ARRAY_ID >= GUILD_FOLDERS.length)) {
        return message.channel.send("Incorrect folder ID given. Try running `rb folder list` to see folders with their IDs.");
      }

      const currFolder = client.folderContents.get(Number(reactionRole.folder_id));
      ...
      if(currFolder) {
        const r = currFolder.roles.find(fr => fr.role_id === reactionRole.role_id);
        if(!r) {
          return message.channel.send(`I can't seem to find that role anywhere...`);
        }
