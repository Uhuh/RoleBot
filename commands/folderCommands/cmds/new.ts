...
export default {
  ...
  run: (message: Message, args: string[], client: RoleBot) => {
    ...
    if (args.length && folderName !== '') {
      ...
      if (folders.length)
        folder = folders.find(
          (f) => f.label.toLowerCase() === folderName.toLowerCase()
        ) || { id: null, label: '' };
      ...
      if (folder.id === null) {
        ...
        client.guildFolders.set(GUILD_ID, [
          ...folders,
          { id: ID[0].id, label: folderName, guild_id: GUILD_ID },
        ]);
        ...
        client.folderContents.set(folder.id, {
          id: folder.id,
          label: folder.label,
          guild_id: GUILD_ID,
          roles: [],
