...
export default {
  desc:
    'All role folders. If an id is given it list the roles within the folder.',
  name: 'list',
  args: '[folder id]',
  type: 'folder',
  run: (message: Message, args: string[], client: RoleBot) => {
    ...
    if (FOLDERS && args.length && !Number.isNaN(Number(args[0]))) {
      ...
      if (roles && roles.length) {
        embed.setDescription(
          roles.map(
            (r) =>
              `${message.guild!.emojis.cache.get(r.emoji_id) || r.emoji_id} - ${
                r.role_name
              }`
          )
        );
      } else embed.setDescription(`No roles in this folder.`);
    ...
    embed.addField(
      '‏‏‎Reaction roles not in a folder.‎',
      `[ Free Roles ] - [${SIZE} ${SIZE > 1 ? `Roles` : `Role`}]`
    );
    ...
    if (FOLDERS && FOLDERS.length) {
      embed.setDescription(
        FOLDERS.map((f, index) => {
          ...
          return `[ ID ${index} ] - 📁**${f.label}** [${folder.roles.length} ${
            folder.roles.length > 1 ? `Roles` : 'Role'
          }]`;
        })
      );
    } else embed.setDescription(`There are no role folders.`);
    ...
    return message.channel.send(embed).then((m) => {
      ...
      // m.react("⬆️");
      m.awaitReactions(
        (r, user) => r.emoji.name === '⬇️' && user.id === message.author.id,
        { maxEmojis: 1 }
      )
        .then(() => {
          const embed = new MessageEmbed();
